...
`art/face-manifest.json` 是那一批的 prompt，去背後放 `art/face/`。

產法跟立繪同一條線（codex、綠幕），差別只有兩點:

- **參考圖是那個角色自己的定稿立繪**，不是畫風參考圖。先把透明的 `sprite-*.png`
  貼到純綠底、縮到 1024px 存成 `art/ref/*-green.jpg`。
  透明 PNG 直接送過去會被當成黑底。
- prompt 第一段先寫死「不可以改的」：角色設計、髮型、五官、服裝配件、配色、
  體型、全身取景與人物比例。**要改的只有臉部表情與手勢。**
  不寫死的話它會順手換一套衣服。

去背跑 `python3 tools/cut_faces.py`，三步:綠幕 key → `tools/interior.py`
清掉被主體圍住的殘留 → 負控制驗收(不透明像素數、殘留綠、四角 alpha)。
**驗收不要貼白底或棋盤格**，看不出整片被吃掉。

| 角色 | 差分 | 用在哪 |
|---|---|---|
| 鐵塔 | 公事／疲憊／難得的溫柔 | 二・報數字、嚼喉糖、「妳今天聲音有點啞」 |
| 0x | 意外／壓著／要走 | 三・「妳為什麼要按」、走廊那句、七・會客室 |
| 斑比 | 不安／被說中／專注 | 四・第四十版、「兩年來第一次」、指著七行問 |
| 諾亞 | 想事情／笑 | 五・「妳這個問題很難」、「因為妳每次都是第一次問啊」 |
| 貓草 | 發酸／彆扭 | 他只在留言區，畫面上用不到，角色頁完整用 |
| 格莉奇 | 難過（新增第七張） | 七・「有五百次」、「那你講給我聽」 |
