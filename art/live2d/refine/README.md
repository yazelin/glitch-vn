...
直線切口掃過胸口。`split_sleeve.py` 把肩縫右側的袖子像素逐像素搬進帽T，袖子只剩手臂本體。
兩個坑：**不能對袖子整體羽化**（外輪廓會透出背景）；**不能直接覆蓋帽T**（帽T 自己也有半透明邊、底下是 58_hood），
要用直通 alpha 的 over 合成 `out_a = a + ha(1−a)`、`out_rgb = (s·a + h·ha·(1−a)) / out_a`。
判準是休息姿態合成完全不變（最大差 3）。搬完重疊 63,066 → 574 px。
