...
3. `larch/RELEASES.md` — 每一版發了什麼、貼進市集後台的文字。**最新：調查篇 1.4，2026-09-18 已發佈，市集 release 5。**

## 現況（2026-09-18 晚）

- 調查篇 Larch 專案 id 在 `larch/inv/state.json`（`project-d2fea918-…`）。正篇是另一個專案（`larch/config.py`），別搞混。
- 線上版子：923 張卡／1057 條線（含 109 張清場卡、13 個群組框），謝幕版子 7／6。素材庫 209 筆、沒引用 0、圖全部 webp、沒有任何網址指到正篇專案。
