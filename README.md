...
**2026-09-07 狀態**：694 張卡、236 條條件邊，推上新專案回讀一致。錄音機制（五卷目擊證詞、
播給管理員聽、HUD 隨時重聽）、她的手機（四頁、通知橫幅）、收尾門檻三條件、選單標籤、立繪與道具圖、保全四場、路人五格、
每日收尾、招呼卡、軟木調查板（拍立得）、白天／晚上／深夜三組背景（每個地點三張入口場景）都進了。
街區定在台北南港（南港車站、二號出口、藍15 與 605、南港路的材料行、中視），對應表在 `design/調查篇-場景.md` 一。
結局：第十二天一開板插播十一天收尾，接著跳進「十二、最後一頁」，演完跳到另一塊版子「謝幕」（放映廳 → 片尾字卷 → 全體上台，照正文第八章的做法；字卷照這一輪的變數排：她記住的那一頁、她見過的人、她去過的地方）。
還沒做的：BGM 與音效（一首都還沒接）、五個新角色的表情差分（台詞裡沒有情緒標記，先不做）、配音。
