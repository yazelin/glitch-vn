...
:root{
  ...
  --text:#dfe8ec; --muted:#93a3ac; --faint:#6d7d84;
  --r:3px;
}
/* ── 有聲書 ──────────────────────────────────────────
...
.views{margin-top:20px;padding-top:16px;border-top:1px solid var(--hair)}
.views h4{margin:0 0 10px;font-size:.72rem;font-weight:400;letter-spacing:.12em;
  color:var(--faint)}
.view{margin-bottom:14px}
.view:last-child{margin-bottom:0}
.view .said{font-size:.82rem}
.ex{margin-top:52px}
...
/* **逐字稿一直顯示，不要按了才出現。** 讀者是來認識角色的，
   那段文字本身就是內容；藏起來只有按下去的人看得到，而且會撐開版面。
   播放中只換左邊那條線的顏色。 */
.said{margin:9px 0 0;font-size:.84rem;line-height:1.85;color:var(--muted);
  border-left:2px solid var(--hair);padding-left:12px}
.said[data-on]{border-left-color:var(--mint);color:var(--text)}
/* **--ink 是深色面板的底色，不是文字色**（文字色是 --text）。
   這顆鈕本來寫成 color:var(--ink)，在 --bg 上等於黑底黑字，
   要 hover 過去才看得見。頁尾的東西本來就不顯眼，更禁不起這種錯。 */
.dl{display:flex;align-items:baseline;gap:4px 12px;flex-wrap:wrap;margin:0 0 16px}
.dl button{font:inherit;font-size:.86rem;color:var(--text);background:none;
  border:1px solid var(--hair2);border-radius:999px;padding:6px 16px;cursor:pointer}
.dl button:hover:not(:disabled){color:var(--mint);border-color:var(--mint)}
.dl button:disabled{opacity:.55;cursor:default}
.dl span{font-size:.8rem;color:var(--muted);font-variant-numeric:tabular-nums}
  ...
  navigator.serviceWorker.ready.then(function (reg) {{
    ...
    function show(d) {{
      ...
      if (d.have >= d.total) {{
        ...
        msg.textContent = '全書 ' + d.total + ' 句都在這台裝置上了';
      }} else {{
        msg.textContent = d.have
          ? '全書 ' + d.total + ' 句配音，已有 ' + d.have + ' 句'
          : '全書 ' + d.total + ' 句配音，約 24 MB。下載之後沒有網路也聽得到';
      }}
    }}
    ask('status', show);
...
(DOCS / "offline.json").write_text(json.dumps({
    ...
    "voice": sorted({d["u"] for d in steps}
                    ...
                    | {f"voice/{f.name}" for f in (DOCS / "voice").glob("view-*.mp3")}),
...
# 別人怎麼說：一個角色對另一個角色的看法，掛在「被講的那個人」的卡片底下。
_views = ROOT / "art/voice/views.json"
VIEWS = json.loads(_views.read_text(encoding="utf-8")) if _views.exists() else []
...
def others(about):
    """別人對這個角色的看法。沒有人講過就整塊不長出來。"""
    rows = [v for v in VIEWS
            if v["about"] == about and (DOCS / f"voice/{v['slug']}.mp3").exists()]
    if not rows:
        return ""
    return ('<div class="views"><h4>別人怎麼說</h4>' + "".join(
        f'<div class="view"><button class="say" data-say="{v["slug"]}" '
        f'data-src="voice/{v["slug"]}.mp3" aria-pressed="false"><b>\u25b6</b>{html.escape(v["who"])}</button>'
        f'<p class="said" id="said-{v["slug"]}">{html.escape(v["text"])}</p></div>'
        for v in rows) + "</div>")
...
def say(slug, name):
    ...
    if not d or not (DOCS / f"voice/intro-{slug}.mp3").exists():
        return ""
    return (f'<button class="say" data-say="{slug}" '
            f'data-src="voice/intro-{slug}.mp3" aria-pressed="false">'
...
DAYS = [
    ...
    (1, "兩點四十", "十一點半該做的事晚了三個多小時。她抄守則本，闔上，關掉客廳最後一盞燈。那一版是第一千零四版，第一章的篇名就是它", "一之三、四", 1),
    ...
    (5, "晚上", "回家之後她往前翻本子。最新那一頁是第一千零七版，往前翻了大概兩百版，翻到撕痕。她算出本子比頻道老", "四之五", 1),
...
每一章多一個支線。</p>
<p class="cta">
<a class="solid" href="https://larch.ink/play/market/a2a10427-7326-4a86-b806-c2476fc1c22a">在 Larch 上玩</a>
...
每一張卡都在版本控制裡。<a href="https://larch.ink">去看看</a>。</p>
</div>''',
