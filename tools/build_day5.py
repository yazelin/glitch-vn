...
b.chain([
    ...
    ("d5m-eye",   "他睜開一隻眼睛，看了她一下，又閉上。", "平常", None),
...
b.link(b.prev, cur); b.prev = cur
b.chain([
    ("d5n-normal", "平常這個時候他在上班。她會拿到一件東西，大聲跟玩家商量，然後選要留、要餵、要交給你保管。", "平常", None),
...
a1 = b.chain([
    ...
    ("d5n-a1-eyes","他的眼睛裡沒有情緒，像在看一件重複了幾百次的例行公事。", "平常", None),
    ("d5n-a1-how", "幾百次？", "發呆", G),
    ("d5n-a1-tap", "他閉上眼。一隻觸手輕拍地板。沒有回答。", "平常", None),
...
a2 = b.chain([
    ...
    ("d5n-a2-wait","他沒有動。房間裡只有她的呼吸聲和窗外偶爾經過的車聲。一分鐘。兩分鐘。她的四千多個位元被沉默填滿，一個一個亮起來又滅掉。", "平常", None),
    ("d5n-a2-ans", "每一天。", "預設", HOLE),
    ("d5n-a2-open","她張開嘴。", "平常", None),
    ("d5n-a2-me",  "每一天我都烤過？", "當機", G),
    ("d5n-a2-you", "每一天妳都問。每一天妳都忘。", "預設", HOLE),
    ("d5n-a2-hold","她站在原地，走不動。她想哭，但她不知道自己在為什麼難過——那個理由不在她這裡。", "發呆", None),
...
a3 = b.chain([
    ...
    ("d5n-a3-qui", "他坐在那裡。她坐在這裡。中間隔著整個房間，沒有人說話。", "平常", None),
...
b.prev = ev
b.chain([
    ("d5e-noback","他沒有出門，也就沒有回來。晚上的流程跟其他天一樣卡住了。", "平常", None),
    ("d5e-allday","你今天整天都在。", "平常", G),
    ("d5e-mm",    "嗯。", "預設", HOLE),
    ("d5e-night", "晚上你要睡嗎？你平常下班回來會做什麼？我忘了，我每天早上都會忘。", "平常", G),
    ("d5e-pause", "他沉默了一下。", "平常", None),
