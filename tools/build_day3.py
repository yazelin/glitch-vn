...
b.chain([
    ...
    ("d3m-warn",  "滿了我還硬要記的話，最舊的那一格會自己掉出去。我攔不住。而且我不會知道掉的是哪一件。", "平常", G),
    ("d3m-pair",  "還有。有時候兩件東西同時放在我腦子裡，我會突然看懂一點什麼。"
                  "分開看就沒有，一定要兩件一起。", "平常", G),
...
for k, (tag, lead, sug) in enumerate(RULEIN):
    ...
    n2 = b.add(f"d3e-in-{tag}", {"type": "input", "title": f"填空位（{tag}）",
        "text": "空位在這裡。今天要留什麼給明天的我？\n"
                "（明天早上的她會照著這句話做。她不會問為什麼——她也不知道要問誰。）",
...
b.chain([
    ...
    ("d3e-ok",    "明天的我會照著這句做。她不會問為什麼，因為她連自己寫過都不知道。", "平常", G),
    ("d3e-hm",    "嗯。", "預設", HOLE),
