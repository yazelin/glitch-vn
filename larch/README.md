...
畫廊上有什麼，寫在 `settings.py` 的 `cgGalleryItems`（`{url, title, locked}`）；
什麼時候解開，寫在版子上，一張 `setVariable` 卡帶 `cgOps`（`novelkit.Chapter.cg`）。
**兩邊都要做。** 只設 `locked` 就是十四張永遠的問號，只給解鎖點就是玩家一開遊戲
就在收藏裡看到「錄音間・第十一次」這種有雷的標題。`verify.py` 會把兩邊對起來數。

    settings.py   cgGalleryItems: [{url, title, locked: True}, …]   ← 一開始鎖著
    build_chNN.py c.cg("bg-booth", "錄音間・第十一次")               ← 走到那一場才開

- **只有 `setVariable` 這個型別的卡吃 `cgOps`。** 對話卡與場景卡上寫了不會報錯，
  只是永遠不解鎖。
- **那張卡的 `text` 不可以留空。** 播放器的自動前進條件是
  `!(mode === "card" || text.trim())`，空白的話卡片模式就是一張點不動的空白卡。
- **播放器是照 `url` 對的**，`cgGalleryItems[i].url` 與 `cgOps[i].url` 要同一個字串。
  解鎖狀態存在讀者瀏覽器的 `localStorage["larch-cg-<專案 id>"]`。
- 解鎖卡的 id 是 `chNN-cgN`，**不佔劇情卡的流水號**：在中間插卡如果把後面整章的
  id 往後推，讀者的存檔就全部對不上了。
- **改完要實際玩過一遍**，靜態檢查代替不了：`OUT=/tmp node tools/autoplay_main.mjs
  <playUrl> <專案 id> 14` 會從標題一路點到第十四張解開，順便在解到第七張的時候
  另開分頁看畫廊當負控制（那時候應該是「已經看過 7 個」，另外七格還是問號）。
- **收的要跟畫面上看到的是同一張。** 「聯動・猜歌」原本指著 `bg-studio-day`
  （同一個房間的白天版，早先畫的），可是第三章那一場的背景後來換成專門畫的
  `bg-collab-studio`，玩家收到的跟看到的不是同一張，2026-09-11 換過來了。
  換網址等於作廢舊的解鎖紀錄（存檔的鍵就是網址本身）。
...
**推版子走 `PUT /boards/:id`，不要走 `PUT /projects`。** 整包專案已經一點四 MB
（八塊版子、七百多個素材），2026-09-11 那天整包 PUT 連著回兩次 502，第三次才過。
版子端點一次只送一章，幾十 KB。**它的回應沒有 `nodes`／`edges`**，要驗推上去沒有
只能再 GET 一次。`settings.py` 還是得整包 PUT（`PUT` 只留你送的欄位），那一支慢是正常的。

**重建版子會把平台上翻的譯文整批刪掉，而且不報錯。** 平台的翻譯功能把譯文寫在每張卡的
`data.localizations` 裡，建置腳本產不出那個欄位，推上去就是「我這一份沒有」＝刪掉。
下一個專案要是有翻譯要保，`push()` 裡要照卡片 id 把線上那一份接回來。
`verify.py` 每章印一個「翻譯 N 張」，就是為了讓這件事看得見。

**這個作品只有正體中文。** 2026-09-11 讀回來的時候第一章有八十一張卡帶著 ja-JP 譯文、
`project.languages` 多一筆日語，是平台上跑過一次翻譯功能留下的，只翻了第一章，作者決定
拿掉。**兩邊都要清**：譯文在版子那一邊（`push()` 不接回來，推一次就沒了），語言清單在
`setup_language.py`。只清一邊會留下殘骸。`zh-Hant` 那一筆的 `voiceMode: "shared"`
一個字都不能動——匯出的單檔播放器只看它，動到語音整個不響。
