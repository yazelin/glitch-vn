...
`larch/cards/park-arcade-demo.py` 建的乾淨示範專案（`project-892df123-4806-4343-95be-44f1c65056e4`，
2026-09-21 建），給別人看 arcade-hub 怎麼接：14 張卡、24 條邊，五款全開。
前兩款接完整的分岔（收齊／未集齊各一張卡，後面各接一張清旗標的 `setVariable`），
後三款六條邊收斂到同一張卡，示範分岔不必一對一；再加離開與無條件保底，插件卡共 12 條出邊。
每張分岔卡的台詞就是說明書，寫明剛才那張卡寫了哪個變量、這條邊判的是什麼、
為什麼不共用一個變量問 `==false`。最後一張列五款的 repo、協定前綴與各自「收齊」的定義。
變量全部 `demo_` 前綴，跟調查篇那組不相干。

**觸發變量用完要清回 `false`。** 調查篇每條邊都還掛著 `__larch_cg__:<hash>==false`，
CG 播過就解鎖，那條邊之後不會再成立；沒有 CG 的專案少了這道閘，玩完 A 款之後
A 的旗標會一直是 true，接著玩 B 款離開時 A 那條邊又會被選中。示範專案因此在每條
分岔後面接一張 `setVariable` 卡把旗標清回去（一張卡可以一次清好幾個變量）。

重跑用 `--project <id>`，不帶參數會再建一個新的（agent API 刪不掉專案，別手滑）。
**沒有發佈到市集**，要上架或開 remix 讓人抄接線是另一個決定。

## 部署與測試踩過的雷

- **插件定義每張卡最多 30 個欄位，超過的靜默砍掉。** 2026-09-21 實測：本機 39 個欄位的
  manifest 貼上去、`GET /api/agent/plugins/arcade-hub` 讀回來只有 30 個，少的正好是排在
  最後那九個（第 4 款的三個變量欄加第 5 款整組六個）。回 200、沒有任何提示，html 與
  description 都完好，只有 `fields` 被截。別人的插件最多 22 個欄位（voice-input 17／
  slide-deck 18／phone-chat 22），所以這條線以前沒人踩到。1.1.0 因此把「一款六個欄位」
  收成「一款一行、六欄用直線分開」，13 個欄位就夠。
  **注意 `GET /api/plugins` 看不到自己剛發布的插件**，要用 `GET /api/agent/plugins`
  （帶金鑰）才列得出來，也才讀得到 `fields`。


- **插件欄位不能取名 `prompt`，平台會把那個鍵從執行期的 `values` 裡拿掉。** 2026-09-21
  實測：卡片上存的 `pluginValues` 有 37 個鍵（回讀確認 `prompt` 在裡面、值也對），
  但卡片在播放器裡收到的 `larch:init` 只有 36 個，少的就是 `prompt`，於是選單上方那句
  「今晚想先玩哪一台？」整句不見。其他同樣沒有預設值的鍵（`background`、`leaveResultVar`、
  十個 `cg_*` 變量）都好好送到，所以不是白名單、不是長度、也不是沒存進去。改名成
  `subtitle` 就正常。**這個坑不會報錯**，畫面只是少一行字，回讀卡片資料也看不出來——
  要在真的播放器裡把 `window.values` 抓出來數才發現。
