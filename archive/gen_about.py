...
LESSONS = [
    ...
    ("驗證", [
        ...
        ("模擬器走得到，不代表線接好了",
         "模擬器只驗「從起點走得到」。斷頭的邊、指向不存在的卡、沒有入邊的孤島、"
         "選項少接一條線——這些它全都看不見，因為那些東西本來就走不到，"
         "不會出現在「走過的卡」裡面。",
         "<b>check_wiring.py</b> 直接驗圖本身。它抓到的東西模擬器一個都沒報過："
         "重複的邊 id（同一組多條件的線全部撞在一起）、"
         "以及「這個出口全部都有條件、沒有保底」。"),
        ("只有一條有條件的出口，等於死路",
    ...
    ("整個前提重做", [
        ("玩法必須符合關係，不是符合主題",
         "「幫她記住」這個主題有很多做法。我提過一個：玩家幫她寫今天的小抄。"
         "作者一句話打掉——<b>那是小編在做的事，粉絲不會寫主播的稿。</b>",
         "玩法要先問一件事：<b>做這件事的人，跟她是什麼關係。</b>符不符合主題是後面的事。"
         "改成在留言區搶答之後，一切都對了：粉絲本來就在做這件事，"
         "而且它剛好解釋了為什麼粉絲名叫「記憶體」。"),
        ("我照著自己的轉述寫人設，寫了七天",
         "角色正典在另一個 repo 的 <code>persona.json</code> 裡。我一直照著自己記得的版本寫，"
         "沒有回去讀原文。原文的 <code>taskNote</code> 明寫「4KB 是自嘲人設，"
         "執行任務時能力完整，不要因此裝笨」——我把整個遊戲寫成了重度失憶。",
         "跨 repo 的設定<b>每次動筆前重讀原文</b>，不要相信自己的轉述。"
         "而且要把正典抄進設計文件，讓下一次審查有東西可以比對。"),
        ("每天玩法一樣，第三天就會想關掉",
         "第一版七天全部都是「卡住、搶答、唸出來」。試玩模擬的回答是"
         "「我只是一直在留言區搶答當工具人，再這樣我真的會想關掉」。",
         "七天七個動詞：搶答／指路／點歌／寫信／只能看／搶答／講出來。"
         "<b>第五天故意把玩家的輸入框關掉</b>——前面四天你一直有事做，"
         "這一天你只能看著她烤麵包、看著她寫下那五個字。"),
        ("結局看錯了變數",
         "結局本來看「玩家保管了幾件」。可是保管了卻一件都不講的玩家，"
         "會拿到「你今天講的那些我都記到格子裡了」。",
         "改成看<b>真的講出來的那幾件</b>。保管是你的事，講出來才是她的事。"),
        ("記憶格掛在已經有無條件出線的卡上，等於沒接",
         "搶答成功那張卡已經有一條無條件的線往「她的反應」。我在後面又接了一條往記憶格——"
         "Larch 只認第一條無條件的線，所以整組記憶格從來沒有走到過。",
         "模擬器一跑就看得出來（那一整組卡零次到達）。"
         "<b>要插在中間就要先 unlink</b>，不能用「多接一條」的方式插隊。"),
        ("放大工具吐全黑圖，退出碼還是 0",
         "三張背景放大完，每一張都是 33KB、大小一模一樣、內容全黑。",
         "存檔前驗<b>平均亮度與標準差</b>。光看檔案大小或退出碼完全看不出來。"
         "1376→1920 只有 1.4 倍，LANCZOS 就夠，不需要放大模型。"),
    ]),
    ("用 AI 幫忙寫", [
        ("提問的框架決定你會拿到什麼答案",
    ...
    ("玩家實測之後", [
        ("「一天過得好快，才幾句話就沒了」",
         "第三天原本 50 張卡，早中晚各十幾句。作者自己玩完說完全沒有經歷過的感覺。"
         "問題不在句數，在於一整天只有一個決定——中午那個三選一做完，這天就結束了。"
         "現在中午是探索：五個地方只陪得了三個，黑洞先生傍晚就回來。同一天變成 130 張卡、"
         "1950 條玩法。",
         "改完要驗的不只是「跑不跑得動」。<b>模擬器要能處理設計上的迴圈</b>——"
         "探索選單會繞回來，那是設計不是環。用 (卡片, 狀態) 去重擋住真正的無限迴圈。"),
        ("「不知道自己在記什麼」",
         "她的記憶體有四格，但玩家從頭到尾沒看過裡面裝了什麼——狀態列只寫「1／4」。"
         "你不可能管理一個看不見的東西。",
         "把四格攤開來，用 <code>{{slot1}}</code>…<code>{{slot4}}</code>。"
         "<b>先確認播放器會對選項文字插值</b>再動手（`re(e)` 有套在選項的 span 上），"
         "不然整個介面是空的。"),
        ("五家會審說「像資源管理作業」",
         "第一版是滿了讓玩家挑要丟哪一格。五個模型獨立說那玩起來像雜務，而且不符合她——"
         "她記不住的東西是自己掉出去的，她沒得挑。",
         "改成<b>最舊的自己掉出去</b>。順序固定，所以掉的是什麼算得出來，"
         "那張卡因此寫得出真正的台詞，不是泛泛的「你忘記了某個東西」。"),
        ("拼出來的東西必須改變明天",
         "線索合併第一版只多幾句獨白，不改變任何事。三家用不同的話說了同一件事："
         "玩家是在替系統整理語音日記，不是在拯救一段關係。",
         "她拼出來的東西<b>變成她寫進守則的那句話</b>，而隔天早上她會唸出來、然後照做。"
         "那是整個記憶機制第一次真的跨過一天。"),
        ("整條線一起審，才看得出哪一天沒跟上",
         "每一天都單獨審過、單獨改過之後，六個模型讀完整條七天的線，"
         "一致指出同一件事：有一天沒有被重做，而且它剛好夾在兩個高潮中間。"
         "單獨看那一天沒問題，放進整條線裡就是個洞。",
         "審完一天不等於審完。<b>整條線要另外審一次</b>，"
         "而且問法要換成「哪一天最弱」「玩到第幾天會關掉」，不要問「這天好不好」。"),
        ("合併要發生在當下，不是事後",
         "第一版是晚上才判定哪兩件線索湊得起來。玩家探索前不知道哪兩個能拼，變成盲賭，"
         "拼不到就白過一天。",
         "搬到<b>留下第二件的當下</b>。走到那條路代表這一件一定留著了，"
         "所以只要比對方一個變數，連 AND 都不用串。"),
    ]),
...
<div class="stat"><b>1</b><span>個結局</span></div>
</div>
</header>

<section>
<div class="eyebrow">這個遊戲</div>
<h2>白天你是幾千分之一，晚上你是唯一的一個</h2>
<p>格莉奇是 VTuber。粉絲的名字就叫「記憶體」——幫偶像記事情是粉絲本來就在做的事。</p>
<p>直播的時候她旁邊有提示詞、有留言區，所以她是清楚的、反應快的、記得住每一個人的。
她卡住的那一秒，留言區刷起來，她挑一則唸出來——<strong>唸出來的才留得住</strong>。
你一天只有三次搶答機會，而她一天會卡住五到六次。</p>
<p>下播之後那些全部關掉。<strong>而她會忘記把麥克風也關掉。</strong>
畫面沒有黑，鏡頭朝著天花板。幾千個人都散了，只剩下沒有把分頁關掉的那一個人。
你看不到她，你看得到光——她忘掉一件事的時候，天花板會亮一下。</p>
<p>黑洞先生跟直播沒有關係。他就是室友，不上鏡，粉絲不認識他，整個人活在下播之後那一段。
他只吃被忘掉的事，吃不了任何食物。</p>
