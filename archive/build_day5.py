...
b.chain([
    ...
    ("d5m-nvm",   "不用管。", "預設", HOLE),
    ("d5m-num",   "等一下。昨天我好像請你幫我記一個數字。", "平常", G),
    ("d5m-num2",  "她走到門邊，蹲下來數。「一、二、三……{{holeFeet}}。」", "平常", None),
    ("d5m-num3",  "今天是 {{holeFeet}}。昨天是多少？你記得嗎？", "平常", G),
    ("d5m-num4",  "她抬頭看你，等一個她自己給不出來的答案。", "平常", None),
...
b.chain([
    ...
    ("d5e-hook1", "今天他一整天都在。這件事我明天不會記得。", "平常", G),
    ("d5e-hook2", "你記得就好。你記得，今天就算真的發生過。", "平常", G),
    ("d5e-lie",   "她躺下來。角落那截影子沒有動。\n"
                  "她閉上眼睛之前，最後看到的是他還睜著的那隻眼睛。", "平常", None),
])
